...
void calculateBoundingBox(glm::vec2& screenPosV0, glm::vec2& screenPosV1, glm::vec2& screenPosV2, int width, int height,
						  int& bbLeftX, int& bbRightX, int& bbBottomY, int& bbTopY)
{
	// compute bounding box
	bbLeftX = std::min(1.0f, std::max(0.0f, std::min(std::min(screenPosV0.x, screenPosV1.x), screenPosV2.x))) * width;
	bbRightX = std::max(0.0f, std::min(1.0f, std::max(std::max(screenPosV0.x, screenPosV1.x), screenPosV2.x))) * width;
	bbBottomY = std::min(1.0f, std::max(0.0f, std::min(std::min(screenPosV0.y, screenPosV1.y), screenPosV2.y))) * height;
	bbTopY = std::max(0.0f, std::min(1.0f, std::max(std::max(screenPosV0.y, screenPosV1.y), screenPosV2.y))) * height;
...
void calculateVariousPos(glm::mat4& modelMat, glm::mat4& viewMat, glm::mat4& projMat, const std::vector<vertex>& vertices,
							glm::vec3& worldPosV0, glm::vec3& worldPosV1, glm::vec3& worldPosV2,
							glm::vec3& viewPosV0, glm::vec3& viewPosV1, glm::vec3& viewPosV2,
							glm::vec4& NDCPosV0WithW, glm::vec4& NDCPosV1WithW, glm::vec4& NDCPosV2WithW, 
							glm::vec3& NDCPosV0, glm::vec3& NDCPosV1, glm::vec3& NDCPosV2, 
							glm::vec2& screenPosV0, glm::vec2& screenPosV1, glm::vec2& screenPosV2)
{
	worldPosV0 = modelMat * glm::vec4(vertices[0].pos, 1.0);
	worldPosV1 = modelMat * glm::vec4(vertices[1].pos, 1.0);
	worldPosV2 = modelMat * glm::vec4(vertices[2].pos, 1.0);
	viewPosV0 = viewMat * glm::vec4(worldPosV0, 1.0);
	viewPosV1 = viewMat * glm::vec4(worldPosV1, 1.0);
	viewPosV2 = viewMat * glm::vec4(worldPosV2, 1.0);
	NDCPosV0WithW = projMat * glm::vec4(viewPosV0, 1.0);
	NDCPosV1WithW = projMat * glm::vec4(viewPosV1, 1.0);
	NDCPosV2WithW = projMat * glm::vec4(viewPosV2, 1.0);
	...
	NDCPosV0 = glm::vec3(NDCPosV0WithW.x / wV0, NDCPosV0WithW.y / wV0, NDCPosV0WithW.z / wV0);
	NDCPosV1 = glm::vec3(NDCPosV1WithW.x / wV1, NDCPosV1WithW.y / wV1, NDCPosV1WithW.z / wV1);
	NDCPosV2 = glm::vec3(NDCPosV2WithW.x / wV2, NDCPosV2WithW.y / wV2, NDCPosV2WithW.z / wV2);
	screenPosV0 = (glm::vec2(NDCPosV0.x, NDCPosV0.y) + glm::vec2(1.0));
	screenPosV1 = (glm::vec2(NDCPosV1.x, NDCPosV1.y) + glm::vec2(1.0));
	screenPosV2 = (glm::vec2(NDCPosV2.x, NDCPosV2.y) + glm::vec2(1.0));
...
{
	glm::vec3 worldPosV0, worldPosV1, worldPosV2;
	glm::vec3 viewPosV0, viewPosV1, viewPosV2;
	glm::vec4 NDCPosV0WithW, NDCPosV1WithW, NDCPosV2WithW;
	glm::vec3 NDCPosV0, NDCPosV1, NDCPosV2;
	glm::vec2 screenPosV0, screenPosV1, screenPosV2;

	calculateVariousPos(modelMat, viewMat, projMat, vertices,
		worldPosV0, worldPosV1, worldPosV2,
		viewPosV0, viewPosV1, viewPosV2,
		NDCPosV0WithW, NDCPosV1WithW, NDCPosV2WithW,
		NDCPosV0, NDCPosV1, NDCPosV2,
		screenPosV0, screenPosV1, screenPosV2);

	
	int bbLeftX, bbRightX, bbBottomY, bbTopY;

	calculateBoundingBox(screenPosV0, screenPosV1, screenPosV2, WIDTH, HEIGHT,
		bbLeftX, bbRightX, bbBottomY, bbTopY);
	...
	for (int i = bbTopY; i < bbBottomY; i++) // i and j starts from top-left
	{
		for (int j = bbLeftX; j < bbRightX; j++)
		{
			...
			{
				// the directional light is not perspective project, so no perspective correction
	...
	for (int i = bbTopY; i < bbBottomY; i++) // i and j starts from top-left
	{
		for (int j = bbLeftX; j < bbRightX; j++)
		{
			...
			if (weight.x >= 0) // test depth and if it is inside triangle and NDC
			{
				...
				mutexes[i * WIDTH + j].unlock();
			}
		}
	}
}

// a test function, test how many pixels are drawn
void drawTriangleTestNumber(std::vector<glm::vec3>& frameBuffer, std::vector<float>& depthBuffer, const std::vector<vertex>& vertices,
	glm::mat4 modelMat, glm::mat4 viewMat, glm::mat4 projMat,
	glm::vec3 cameraPos, DirectionalLight light,
	const std::vector<float>& shadowMapBuffer, glm::mat4 MVMatLight, glm::mat4 MVPMatLight, bool drawShadow,
	const std::vector<glm::vec3>& texAlbedo, const std::vector<glm::vec3>& texSpecular, bool drawTex,
	glm::vec3 color, bool pureColor, int& testNumber)
{
	...
	for (int i = bbTopY; i < bbBottomY; i++) // i and j starts from top-left
	{
		for (int j = bbLeftX; j < bbRightX; j++)
		{
			...
			if (weight.x >= 0) // test depth and if it is inside triangle and NDC
			{
				...
				float zp = 1 / (weight.r / z0 + weight.g / z1 + weight.b / z2);
				float depth = zp;
				//std::cout << depth << "   " << depthBuffer[i * WIDTH + j] << std::endl;
				if (depth >= depthBuffer[i * WIDTH + j] && depth <= 0)
				{
					testNumber++;
					if (!pureColor)
